    ...
    for key in CEILINGS + ["any_basis"]:
        ...
        for label, f1, f2, minutes in _intervals(fcurve):
            ...
            rows.append({
                ...
                "CONTINUOUS_HAZARD_LAMBDA": _lam(f1, f2, minutes),
...
def consensus(priors):
    ...
    out = {
        ...
        "CONSENSUS_WEIGHTING": ("FIRST_SIDE_ACQUISITIONS" if have_all else NI),
        "CONSENSUS_WEIGHTS": weights if have_all else NI,
    ...
    for i, r0 in enumerate(rows0):
        lams = [p["HAZARD_BY_BASIS_CEILING"]["any_basis"]["ROWS"][i]
                ["CONTINUOUS_HAZARD_LAMBDA"] for p in usable]
        if any(l == NI for l in lams) or not have_all:
            lam = NI
        else:
            lam = sum(l * weights[p["ACCOUNT"]]
                      for l, p in zip(lams, usable)) / wsum
        out["HAZARD_any_basis"].append({
            "INTERVAL": r0["INTERVAL"],
            "CONSENSUS_CONTINUOUS_HAZARD_LAMBDA": lam,
            "MEMBER_LAMBDAS": dict(zip([p["ACCOUNT"] for p in usable], lams)),
