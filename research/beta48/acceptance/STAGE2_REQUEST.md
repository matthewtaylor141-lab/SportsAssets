...
forecast, and it is not a probability for this batch either**. That corpus is
twelve distinct markets sampled repeatedly over eight days — the reads are not
independent of one another and not a cross-section of the venue on any given
evening — while the probe's 40 candidates come from a deterministic
slug-ordered cursor over the live listing. Neither independence nor
representativeness has been demonstrated, so no point estimate (and no earlier
"~2.5 markets" figure of mine) follows from it. The probe's yield is simply
**not established**. Zero eligible markets is a legitimate outcome and so is
forty; the run reports what happened rather than scoring it against a
prediction. If selection returns no eligible markets within the 40-candidate
budget, that outcome is **reported and the probe stops**. The
