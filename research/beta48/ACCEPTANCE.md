...
| 1.5 | executable same-venue price AND depth | ✓ | ✓ | ✓ | ⏳ | the stage runs on production data and refuses, and **run 22 (02:33:10Z) named which of the three causes it is: `VENUE_BOOK_READ_RETURNED_ERROR 2`** — the venue answered the book read with an error for both mapped contracts. `VENUE_ASK_HAS_NO_DEPTH`, `VENUE_QUOTE_STALE` and `VENUE_MARKET_ROW_HAS_NO_SLUG` did not fire. The cycle now keeps the venue's own message (bounded to 5, deduplicated, slug-tagged) so the next read says whether that is an entitlement, a closed market or a rate limit |
