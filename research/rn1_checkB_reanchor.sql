-- ============================================================================
-- CHECK B RE-ANCHOR: the scan clock, and whether the placement matters
-- (2026-09-11, read-only.)
--
-- THE SNAPSHOT IS NOT ATOMIC. Four questions were put to the code before any
-- row here was treated as a point-in-time anchor, and all four answer against
-- atomicity (whale_exits.py:305-400, :1102-1108):
--
--   1 PER-PAGE TIMING PERSISTED?  NO. The blob is
--       {"at": time.time(), "partial": bool, "sizes": {token: size}}
--     written ONCE after the loop (:1107). No per-page, per-request or
--     scan-start timestamp is recorded anywhere, and the heartbeat carries
--     per-cycle stats that are overwritten and not joinable to a shadow row.
--     SCAN-START TIMING THEREFORE CANNOT BE RECONSTRUCTED, so no real earlier
--     boundary exists and no true interval can be reported.
--   2 PAGE INDEX RETAINED PER POSITION?  NO. Rows are accumulated into one
--     flat dict (`out[a] = sz`, :391); page membership is discarded as it goes.
--   3 CAN A CONDITION'S TWO TOKENS LAND ON DIFFERENT PAGES?  YES. The request
--     carries no ordering parameter and nothing groups a condition's legs;
--     RN1's book is 10k+ rows (:972) = 20+ pages at 500/page. The two legs are
--     separate rows -- `_sibling_of` exists precisely because they are. So
--     snap_long and snap_other MAY BE OBSERVED AT DIFFERENT MOMENTS, and the
--     net snap_long - snap_other can mix two instants.
--   4 SERVER-SIDE SNAPSHOT OR CURSOR SEMANTICS?  NO. It is OFFSET pagination
--     (`offset += len(rows)`, :392) with no cursor, no as-of, no snapshot id.
--     Offset paging over a mutating collection can skip or duplicate rows; a
--     consistent read is NOT implied by pagination and the code never claims
--     one. At the 6 rps throttle the walk's WAIT ALONE is >= 8 s for 48 pages
--     (the E10 comment at :345-352), so the scan spans seconds to tens of
--     seconds.
--
-- THEREFORE THE EVIDENCE CONCEPT IS RENAMED. This is SCAN_ANCHORED_STATE:
-- non-atomic, independently observed state. The claim it supports is
--     "RN1 state independently observed DURING A SCAN ending no later than
--      blob.at"
-- and NOT
--     "RN1 state at time T = snap".
-- Nothing downstream may upgrade it to a point-in-time observation.
--
-- WHAT snap_age_s IS, from the persistence path:
--   shadow.at    TIMESTAMPTZ DEFAULT now() (migration 046) -- the DB INSERT
--                instant, after that tick's mapping and a paced quote call.
--   blob "at"    time.time() when the blob is WRITTEN, i.e. when the walk
--                COMPLETED (whale_exits.py:1107).
--   snap_age_s   time.time() - blob.at, evaluated when mirror_shadow reads the
--                blob (mirror_shadow.py:740), stored to 0.1 s at :2535.
--   freshness    snap_age_s <= SNAP_MAX_AGE_S, default 300 s, floor 120 s
--                (:128, :2491); a stale read persists snap_long NULL (:2511).
--
--     scan_start  <=  scan_end (= blob.at)  <=  at - snap_age_s  <  at
--                 \______ unobservable ______/
--
-- So at - snap_age_s is a SCAN-COMPLETION-DERIVED, LATEST-DEFENSIBLE placement.
-- It is not an observation timestamp. The gap back to scan_start is real,
-- unrecorded, and NOT bounded by anything persisted -- which is exactly why
-- variant 2 below is labelled arbitrary rather than a bound.
--
-- THE TWO CLOCKS STAY SEPARATE.
--     snapshot_effective_at    = at - snap_age_s   (state)
--     designation_effective_at = at                (decision; DOES NOT MOVE)
-- _choose_long(assets, cands, pos, source) takes pos = mi.net_positions(fills)
-- (mirror_shadow.py:946) -- OUR FILLS-DERIVED POSITION AT TICK TIME. It never
-- reads the scan. Backdating the designation would assert a decision existed
-- before the data it was made from. anchor_long orients a row's own two
-- numbers into outcome-index space and is never a designation for any event.
-- No future designation classifies an earlier fill.
--
-- FIVE STATE MODELS, one population, one causal designation, so every
-- difference between them is state and nothing else. O uses PLACEMENT_LATEST
-- so O against C isolates the REBASING POLICY; D against C isolates the
-- PLACEMENT, both over an identical event set (ga0 > 0 = run 41's set).
--
-- THE QUESTION THIS ANSWERS: are the BUY/SELL/HOLD conclusions STABLE across
-- the defensible placements? If they move materially, the reconciled model is
-- INTERVAL-IDENTIFIED, not point-identified, and must be reported that way. If
-- they barely move, the clock imperfection is not economically material.
--
-- Read-only: three SELECTs. Nothing here writes.
-- ============================================================================


\echo '== 1. THE CLOCK ITSELF: what the persistence path actually recorded =='
...
\echo '== 2. BUY/SELL/HOLD MIX under each of the five state models =='
WITH ac AS (
...
\echo '== 3. PAIRWISE: the two the owner asked for, the clock delta, and the stress =='
