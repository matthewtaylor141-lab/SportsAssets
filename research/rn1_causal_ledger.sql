...
--
-- ---------------------------------------------------------------------------
-- P1 IS SEPARATED FROM P2/E12, AND THE BOUNDARY IS DERIVED, NOT ASSUMED.
--
-- Run 73 evidenced P1 directly: on 2026-09-02 through 09-05 EVERY observed
-- negative-net production tick recorded target zero rather than a negative
-- target (3,652 / 13,011 / 13,312 / 1,890, none short). That is production
-- behaviour, not a hypothesis about it.
--
-- The regime boundary used below is THE FIRST RECORDED NEGATIVE TARGET, read
-- out of mirror_shadow itself rather than taken from a deploy date. An event
-- before it is in the P1 regime; an event after it is not, however much it
-- resembles one. A post-boundary collapse to zero is counted separately as
-- post_P1_collapse_other_cause and is NEVER labelled a P1 forced flatten --
-- P2 admitted negative targets and E12 changed sizing, so the same shape has a
-- different cause there and the counterfactual does not transfer.
--
-- THE P1 DAMAGE UNIT, exactly as specified:
--     an existing POSITIVE BETTOR position
--     -> the recorded target collapses to ZERO because RN1's signed state went
--        negative and P1 cannot represent a negative target
--     -> actual liquidation
...
--
-- Statement 8 emits ONE ROW PER HORIZON and never blends them:
...
--
-- Statement 9 reports five candidate reconstructions of
...
--
-- ---------------------------------------------------------------------------
-- ACCEPTANCE. Statement 10 runs all seven checks, and EVERY zero-violation
...
WITH base AS MATERIALIZED (
  ...
  SELECT min(at) AS t0, max(at) AS t1,
         min(at) FILTER (WHERE target < 0) AS p1_end
    FROM sh
  ...
  SELECT e.*, s.first_seen, s.last_seen,
         CASE WHEN e.has_tick_before AND e.has_tick_after THEN '1 IN_REACH'
              ...
              ELSE '5 OUTSIDE_REACH: observed, but no tick after the event'
         END AS reach
  ...
  SELECT t.*,
         ...
         (t.event_ts < COALESCE((SELECT p1_end FROM win), 'infinity'::timestamptz))
           AS in_p1_regime
    FROM tcl t
  ...
  SELECT c.condition_id, c.fill_id, c.event_ts, c.fsh, c.fpx, c.reach,
         ...
         c.in_p1_regime, c.target_before, c.target_after, c.ledger_before,
...
SELECT COALESCE(recorded_target_transition, 'NULL (no straddling tick)')
         AS recorded_target_transition,
       count(*) AS events,
       count(DISTINCT condition_id) AS conditions,
...
SELECT plan_side, wire_intent, kind, orders, orders_with_a_fill,
       ...
       CASE WHEN signed_long_delta > 0 THEN 'ADDS long-axis exposure'
            ...
            ELSE 'net zero' END AS economic_direction,
...
SELECT recorded_target_transition, actual_executed_transition,
       ...
       round(sum(exec_notional)::numeric, 2) AS executed_notional,
       round(sum(exec_realized)::numeric, 2) AS executed_realized
  FROM ay GROUP BY 1, 2 ORDER BY 3 DESC LIMIT 40;

\echo '== 6. THE P1 REGIME BOUNDARY, derived from the data, and the P1 population =='
WITH base AS MATERIALIZED (
  SELECT t.id, t.tx_hash, t.asset, t.ts, t.condition_id, t.outcome_index,
         t.size::float8 AS sh, t.price::float8 AS px, t.side,
         CASE WHEN t.source IN ('poll', 'backfill') THEN 'venue' ELSE 'cash' END AS feed
  ...
  SELECT e.*,
         (e.in_p1_regime AND COALESCE(e.ledger_before, 0) > 0
          AND e.target_after = 0 AND COALESCE(e.net_after, 0) < 0)
           AS p1_forced_flatten,
         (NOT e.in_p1_regime AND COALESCE(e.ledger_before, 0) > 0
          AND e.target_after = 0 AND COALESCE(e.net_after, 0) < 0)
           AS post_p1_collapse_other_cause
    FROM exj e
)
SELECT to_char((SELECT t0 FROM win), 'YYYY-MM-DD HH24:MI') AS first_shadow_tick,
       to_char((SELECT p1_end FROM win), 'YYYY-MM-DD HH24:MI')
         AS first_NEGATIVE_target_recorded,
       ...
       count(*) FILTER (WHERE in_p1_regime) AS events_in_the_P1_regime,
       count(*) FILTER (WHERE in_p1_regime AND reach = '1 IN_REACH')
         AS P1_events_in_causal_reach,
       count(*) FILTER (WHERE COALESCE(ledger_before, 0) > 0) AS cond_pre_position_gt_0,
       count(*) FILTER (WHERE target_after = 0) AS cond_target_after_0,
       count(*) FILTER (WHERE COALESCE(net_after, 0) < 0) AS cond_rn1_net_negative,
       count(*) FILTER (WHERE COALESCE(ledger_before, 0) > 0 AND target_after = 0
                          AND COALESCE(net_after, 0) < 0) AS all_three_conditions,
       count(*) FILTER (WHERE p1_forced_flatten) AS P1_FORCED_FLATTEN_EVENTS,
       count(DISTINCT condition_id) FILTER (WHERE p1_forced_flatten)
         AS P1_forced_flatten_conditions,
       count(*) FILTER (WHERE post_p1_collapse_other_cause)
         AS post_P1_collapse_EXCLUDED_not_P1
  FROM p1;

\echo '== 7. P1 FORCED-FLATTEN DAMAGE DETAIL (witness count first) =='
WITH base AS MATERIALIZED (
  SELECT t.id, t.tx_hash, t.asset, t.ts, t.condition_id, t.outcome_index,
         t.size::float8 AS sh, t.price::float8 AS px, t.side,
         CASE WHEN t.source IN ('poll', 'backfill') THEN 'venue' ELSE 'cash' END AS feed
  ...
  SELECT p.condition_id, p.fill_id, p.event_ts,
    ...
    LEFT JOIN ords o ON o.condition_id = p.condition_id
                    ...
                    AND o.placed_at <  p.event_ts + interval '900 seconds'
                    AND o.filled > 0 AND o.avg_px IS NOT NULL
   WHERE p.p1_forced_flatten
   GROUP BY 1, 2, 3, 4, 5
), rq AS MATERIALIZED (
  SELECT l.condition_id, l.fill_id, l.event_ts, l.pre_position,
         l.commanded_shed_shares, l.liq_shares, l.liq_proceeds, l.last_liq_at,
...
SELECT count(*) AS p1_forced_flatten_events,
       count(DISTINCT condition_id) AS conditions,
       ...
       round(sum(LEAST(liq_shares, reacq_shares)
                 * (COALESCE(p_buy, 0) - COALESCE(p_sell, 0)))
             FILTER (WHERE reacq_shares > 0 AND liq_shares > 0)::numeric, 2)
         AS realized_round_trip_loss_where_identifiable,
       round(avg(p_sell)::numeric, 4) AS mean_liquidation_price,
       round(avg(p_buy)::numeric, 4) AS mean_reacquisition_price
  FROM cf;

\echo '== 8. NO_P1_FORCED_FLATTEN_COUNTERFACTUAL, one row per VALUATION HORIZON =='
WITH base AS MATERIALIZED (
  SELECT t.id, t.tx_hash, t.asset, t.ts, t.condition_id, t.outcome_index,
         t.size::float8 AS sh, t.price::float8 AS px, t.side,
         CASE WHEN t.source IN ('poll', 'backfill') THEN 'venue' ELSE 'cash' END AS feed
...
SELECT valuation_horizon,
       ...
       round(sum(reacq_shares * COALESCE(p_buy, 0))::numeric, 2)
         AS actual_path_reacq_cost,
  ...
  FROM hz GROUP BY 1 ORDER BY 1;

\echo '== 9. BRIDGE TO THE BUSINESS PROBLEM: total observed BETTOR loss, and the ratio =='
WITH base AS MATERIALIZED (
  SELECT t.id, t.tx_hash, t.asset, t.ts, t.condition_id, t.outcome_index,
         t.size::float8 AS sh, t.price::float8 AS px, t.side,
         CASE WHEN t.source IN ('poll', 'backfill') THEN 'venue' ELSE 'cash' END AS feed
  ...
  SELECT 'A mirror_books.own_book_pnl' AS source,
  ...
  UNION ALL
  SELECT 'E live_orders.pnl lane=mirror', count(*),
  ...
  SELECT
    COALESCE(sum(LEAST(liq_shares, reacq_shares) * (p_buy - p_sell))
             FILTER (WHERE reacq_shares > 0 AND liq_shares > 0), 0)
      AS p1_damage_reacq_horizon
    FROM cf
), oth AS MATERIALIZED (
  SELECT COALESCE(sum(e.exec_realized), 0) AS other_reversal_realized,
         count(*) AS other_reversal_events
    FROM p1 e
   WHERE e.harmful_command AND NOT e.p1_forced_flatten AND e.executions > 0
)
SELECT c.source, c.rows_in_window, c.rows_null,
       ...
       round((SELECT p1_damage_reacq_horizon FROM dmg)::numeric, 2)
         AS identified_p1_forced_flatten_damage_reacq_horizon,
       ...
       CASE WHEN (SELECT max(total_pnl) - min(total_pnl) FROM cand)
            ...
            THEN 'RATIO NOT PRODUCED: the candidate denominators disagree by more '
                 || 'than 5%, so TOTAL_OBSERVED_BETTOR_LOSS is not reconstructible '
                 || 'consistently from retained data'
            ELSE 'candidates agree within 5%' END AS denominator_verdict
  FROM cand c ORDER BY 1;

\echo '== 10. ACCEPTANCE CHECKS, each with a NON-EMPTY witness count beside it =='
WITH base AS MATERIALIZED (
  SELECT t.id, t.tx_hash, t.asset, t.ts, t.condition_id, t.outcome_index,
         t.size::float8 AS sh, t.price::float8 AS px, t.side,
         CASE WHEN t.source IN ('poll', 'backfill') THEN 'venue' ELSE 'cash' END AS feed
  ...
  SELECT o.book_id, o.placed_at, o.ledger_at_place, o.signed_long_delta,
         lag(o.ledger_at_place) OVER (PARTITION BY o.book_id ORDER BY o.placed_at, o.order_id)
           AS prev_ledger,
         lag(o.signed_long_delta) OVER (PARTITION BY o.book_id ORDER BY o.placed_at, o.order_id)
           AS prev_delta
    FROM ords o
)
SELECT
  -- 1. every harmful-command event in exactly one damage-identification class
  ...
  -- 2. no OUTSIDE_REACH event inside a damage denominator
  ...
  -- 3. wire_intent vs the ledger movement production itself recorded
  ...
  -- 4. P1 forced flatten carries all three conditions
  (SELECT count(*) FROM p1 WHERE p1_forced_flatten) AS chk4_witness_p1_events,
  (SELECT count(*) FROM p1 WHERE p1_forced_flatten
     AND NOT (COALESCE(ledger_before, 0) > 0 AND target_after = 0
              AND COALESCE(net_after, 0) < 0)) AS chk4_violations_expected_0,
  -- 5. no P2/E12 event labeled P1
  (SELECT count(*) FROM p1 WHERE post_p1_collapse_other_cause)
    AS chk5_witness_post_p1_collapses,
  (SELECT count(*) FROM p1 WHERE p1_forced_flatten AND NOT in_p1_regime)
    AS chk5_p2_e12_labeled_p1_expected_0,
  -- 6. the damage aggregation closes over the harmful-command universe
  ...
  -- 7. the transition classifier leaves nothing unnamed
