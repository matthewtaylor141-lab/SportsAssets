...
LOOPS: list[tuple[str, Callable[[], Awaitable[None]]]] = [
    ...
    #
    # DEREGISTERED 2026-09-22. The rollback for the repair at
    # fd39a0a9, and deliberately the bluntest one available: the
    # supervisor cannot start what it is not handed.
    #
    # Reach for this only when the database control is not enough
    # -- when `obs-stop` cannot be written, or a running process is
    # not honouring it. For every ordinary stop, `render-ops sql
