...
NULL-block (old-rule) books counted apart. The pins: the two statements
and their columns, read-only, its place after `books-new` in the case
block and the help line (the E8 / hourly order pins untouched).
"""
...
def test_flow_books_reads_the_block_the_reference_and_both_verdicts_off_the_row():
    ...
    assert len(stmts) == 2 and to == 30000
    rows, census = stmts
...
def test_flow_books_is_read_only_and_sits_after_books_new_with_the_order_pins_untouched():
    ...
    assert "books-new|flow-books|nf-his|" in line and "nf-venue|exits-paired|hourly (got" in line
    hourly, _ = _preset(text, "hourly")
    assert "flow_base" not in hourly, "the hourly line is the five presets' SQL and no more"
