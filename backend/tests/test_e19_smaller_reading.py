...
def test_e19_the_census_name_the_docs_and_no_knob():
    ...
    assert keys[keys.index("drift_smaller_open") - 14] == "disagree_fill_ambiguous"
    assert keys[keys.index("drift_smaller_open") - 15] == "disagree_fill_unexplained"
    assert keys[keys.index("drift_smaller_open") - 16] == "disagree_fill_unread"
    assert keys[keys.index("drift_smaller_open") - 17] == "disagree_fill_adopted"
    assert keys[keys.index("drift_smaller_open") - 18] == "cancel_fill_unread"
    assert keys[keys.index("drift_smaller_open") - 19] == "cancel_fill_late"
    ...
    assert keys[keys.index("drift_smaller_open") - 20] == "fast_status_unread"
    assert keys[keys.index("drift_smaller_open") - 21] == "fast_add_took"
    assert keys[keys.index("drift_smaller_open") - 22] == "fast_add_replaced"
    assert keys[keys.index("drift_smaller_open") - 23] == "fast_add_kept"
    assert keys[keys.index("drift_smaller_open") - 24] == "fast_his_add"
    assert keys[keys.index("drift_smaller_open") - 25] == "fast_order_open"
    ...
    assert keys[keys.index("drift_smaller_open") - 1] == "ledger_stale_refused"
    assert keys[keys.index("drift_smaller_open") - 2] == "ledger_stale_reread"
    assert keys[keys.index("drift_smaller_open") - 3] == "walk_row_gone"
    assert keys[keys.index("drift_smaller_open") - 4] == "walk_row_unread"
    assert keys[keys.index("drift_smaller_open") - 5] == "walk_row_moved"
    assert keys[keys.index("drift_smaller_open") - 6] == "exit_flap_averted"
    assert keys[keys.index("drift_smaller_open") - 7] == "exit_confirm_expired"
    assert keys[keys.index("drift_smaller_open") - 8] == "exit_confirmed"
    assert keys[keys.index("drift_smaller_open") - 9] == "exit_unconfirmed"
    assert keys[keys.index("drift_smaller_open") - 10] == "hand_ambiguous"
    assert keys[keys.index("drift_smaller_open") - 11] == "hand_unread"
    assert keys[keys.index("drift_smaller_open") - 12] == "hand_adopted"
    assert keys[keys.index("drift_smaller_open") - 13] == "hand_explained"
    assert keys[keys.index("drift_smaller_open") - 26] == "turn_woke_fast"
    assert keys[keys.index("drift_smaller_open") - 27] == "cand_market_closed_db"
    ...
    assert keys[keys.index("drift_smaller_open") - 28] == "lost_fill_ambiguous"
    assert keys[keys.index("drift_smaller_open") - 29] == "lost_fill_unexplained"
    assert keys[keys.index("drift_smaller_open") - 30] == "lost_fill_unread"
    assert keys[keys.index("drift_smaller_open") - 31] == "lost_fill_adopted"
    assert keys[keys.index("drift_smaller_open") - 32] == "reopen_refused"
    assert keys[keys.index("drift_smaller_open") - 33] == "he_holds_unread"
    assert keys[keys.index("drift_smaller_open") - 34] == "he_holds"
    assert keys[keys.index("drift_smaller_open") - 35] == "fill_answers_absent"
    assert keys[keys.index("drift_smaller_open") - 36] == "fill_answer_write_failed"
    assert keys[keys.index("drift_smaller_open") - 37] == "order_open_his_exit"
    assert keys[keys.index("drift_smaller_open") - 38] == "cover_in_band"
    assert keys[keys.index("drift_smaller_open") - 39] == "exit_take_in_band"
    assert keys[keys.index("drift_smaller_open") - 40] == "take_in_band"
    assert keys[keys.index("drift_smaller_open") - 41] == "exit_take_rested"
    assert keys[keys.index("drift_smaller_open") - 42] == "wrong_sign_hold"
    assert keys[keys.index("drift_smaller_open") - 43] == "adopt_prior_venue_settled"
