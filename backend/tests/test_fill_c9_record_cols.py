...
def test_c9_no_census_name_the_three_heartbeat_keys_the_plan_field_and_no_order_path_reads_the_columns():
    ...
    assert keys[-40] == "cand_market_closed_db"
    assert tuple(keys[-44:-40]) == ("lost_fill_adopted", "lost_fill_unread", "lost_fill_unexplained", "lost_fill_ambiguous")
    assert tuple(keys[-47:-44]) == ("he_holds", "he_holds_unread", "reopen_refused")
    assert keys[-39] == "turn_woke_fast" and keys[-38] == "fast_order_open" and keys[-33] == "fast_status_unread"
