...
PARTS = ("mirror-tick", "tick-ring", "mirror-pnl", "paired-day", "paired-ratio", "latency-census", "fills-answered",
         "fills-missed", "take-band", "maker-rests", "on-target-why")
...
def test_the_hourly_preset_is_the_nine_presets_sql_joined_under_section_markers():
    ...
    body pins ELEVEN since E31 (TEN since FILL lane 14) -- every caller that
    re-runs it by name (fill-answers, lane 9's record pins) reads the same
    tuple."""
    text = YML.read_text()
    ...
    assert hourly == expected, "the hourly line is the eleven presets' SQL joined; regenerate it"
    markers = re.findall(r"SELECT '== ([a-z-]+)' AS section;", hourly)
    assert tuple(markers) == PARTS
    # FILL lane 14: tick-ring rides right after mirror-tick; E31: the hourly is eleven
    assert markers.index("tick-ring") == markers.index("mirror-tick") + 1 and len(markers) == 11
    assert markers.index("maker-rests") == markers.index("take-band") + 1
...
def test_the_hourly_carries_fill_lane_8s_two_edits_in_its_copies():
    ...
    h = _sql(text, "hourly")
    tb = h[h.index("SELECT '== take-band' AS section; "):h.index("SELECT '== maker-rests' AS section; ")]
    ...
    assert len(PARTS) == 11 and h.count("AS section;") == 11
    # E31: maker-rests' two statements sit between take-band's four and on-target-why
    mr = h[h.index("SELECT '== maker-rests' AS section; "):h.index("SELECT '== on-target-why' AS section; ")]
    assert mr.count(";") == 3 and "GROUP BY ROLLUP (side_w, leg, clause)" in mr
