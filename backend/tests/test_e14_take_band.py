...
def test_e14_the_census_name_sits_before_drift_smaller_open_and_the_pins_hold():
    ...
    # FILL lane 16 placed its one `turn_woke_fast` after FILL lane 11's (+14), then E21 (FILL lane 10) its six fast_* names (+15 .. +20; the key at +21)
    assert keys[keys.index("take_in_band") + 14] == "turn_woke_fast"
    assert keys[keys.index("take_in_band") + 15] == "fast_order_open"
    assert keys[keys.index("take_in_band") + 21] == "drift_smaller_open"
    ...
    # FILL lane 3 (three names), T2 (two), FILL lane 5 (three), E22 (FILL lane 22, four), FILL lane 11 (one) and E21 (FILL lane 10, six) placed theirs after this one (-14 -> -33) -- FILL lane 16 (one name, turn_woke_fast) landed first, so every index here moved by one more
