...
def test_review_q9_the_collapse_rule_never_reads_detected_at_and_the_pins_moved_by_the_arithmetic():
    ...
    # E21 (FILL lane 10) by its six fast_* names (-105 -> -111, -106 -> -112) -- FILL lane 16 (one name, turn_woke_fast) landed first, so every index here moved by one more
    assert keys[-112:-108] == ("books_unreadable", "ratio_stepped", "under_min_notional", "shadow_check_skipped")
    assert keys[-113] == "short_share_cap" and keys[-8:-4] == ("fast_tick", "fast_tick_placed", "fast_tick_skipped", "fast_tick_failed")
