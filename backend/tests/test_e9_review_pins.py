...
def test_review_q9_the_collapse_rule_never_reads_detected_at_and_the_pins_moved_by_the_arithmetic():
    ...
    # eight, E19 by its one and L7 by its one `event_stale` (-69 -> -89, -70 -> -90), E20 by its one `wrong_sign_hold` and E14b (FILL lane 1) by its one
    # `exit_take_rested` (-89 -> -91, -90 -> -92), the convention every builder followed; E9's four stay keys[-8:-4]
    assert keys[-91:-87] == ("books_unreadable", "ratio_stepped", "under_min_notional", "shadow_check_skipped")
    assert keys[-92] == "short_share_cap" and keys[-8:-4] == ("fast_tick", "fast_tick_placed", "fast_tick_skipped", "fast_tick_failed")
